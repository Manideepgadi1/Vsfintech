...
# Multi Chart SAFE Deployment Script
# This script will NOT disturb any existing applications
# Frontend: /multi-chart/ | Backend Port: 9006 | API: /api/multi-chart/
# Server: 82.25.105.18

set -e  # Exit on error

echo "🚀 Multi Chart SAFE Deployment"
echo "======================================"
echo ""

# Configuration
APP_NAME="multi-chart"
BACKEND_NAME="multi-chart-backend"
APP_DIR="/var/www/vsfintech/MultiChart"
BACKEND_PORT=9006
...
if [ ! -d "$APP_DIR" ]; then
    ...
    echo "Please upload your Multi Chart code to $APP_DIR first"
    exit 1
fi
...
HAS_BACKEND=false
if [ -d "$APP_DIR/backend" ] && ([ -f "$APP_DIR/backend/server.py" ] || [ -f "$APP_DIR/backend/app.py" ] || [ -f "$APP_DIR/backend/main.py" ]); then
    HAS_BACKEND=true
...
if [ "$HAS_BACKEND" = true ]; then
    ...
    if [ -f "requirements.txt" ]; then
        ...
        pip install --upgrade pip -q
        pip install flask gunicorn flask-cors pandas numpy plotly -q
    fi
...
if [ "$IS_REACT" = true ]; then
    ...
    echo "Configuring Vite for /multi-chart subpath..."
    VITE_CONFIG=""
    ...
    if [ -n "$VITE_CONFIG" ]; then
        ...
        if grep -q "base:" "$VITE_CONFIG"; then
            sed -i "s|base:.*|base: '/multi-chart',|" "$VITE_CONFIG"
        else
            sed -i "/export default defineConfig({/a\\  base: '/multi-chart'," "$VITE_CONFIG"
        fi
    ...
    if [ -f "src/main.tsx" ]; then
        ...
        if grep -q "<BrowserRouter>" src/main.tsx && ! grep -q 'basename=' src/main.tsx; then
            sed -i 's|<BrowserRouter>|<BrowserRouter basename="/multi-chart">|' src/main.tsx
    ...
    # Create .env file
    if [ "$HAS_BACKEND" = true ]; then
        echo "VITE_API_URL=/api/multi-chart" > .env
...
if [ "$HAS_BACKEND" = true ]; then
    cat > /tmp/multi-chart-nginx.conf << 'EOF'
    ...
    location /api/multi-chart/ {
        ...
        proxy_set_header X-Forwarded-Prefix /multi-chart;
...
EOF
    ...
    # =====================================================
    # MULTI CHART - Advanced Charting Tool (Static)
    # =====================================================
    location /multi-chart/ {
        alias FRONTEND_PATH_PLACEHOLDER/;
        try_files $uri $uri/ /multi-chart/index.html;
...
sed -i "s|FRONTEND_PATH_PLACEHOLDER|$FRONTEND_PATH|g" /tmp/multi-chart-nginx.conf

# Check if Multi Chart config already exists
if grep -q "MULTI CHART" "$NGINX_CONFIG"; then
    echo -e "${YELLOW}⚠️  Multi Chart config already exists in nginx${NC}"
    ...
    if grep -q "location / {" "$NGINX_CONFIG"; then
        sed -i '/location \/ {/i\    # MULTI CHART CONFIGURATION' "$NGINX_CONFIG"
        sed -i '/# MULTI CHART CONFIGURATION/r /tmp/multi-chart-nginx.conf' "$NGINX_CONFIG"
        sed -i '/# MULTI CHART CONFIGURATION/d' "$NGINX_CONFIG"
        echo -e "${GREEN}✓ Nginx configuration added${NC}"
    else
        echo -e "${YELLOW}⚠️  Could not automatically add nginx config${NC}"
        echo "Please manually add the config from: /tmp/multi-chart-nginx.conf"
    fi
fi
...
# Test frontend
FRONTEND_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost/multi-chart/ 2>/dev/null || echo "000")
...
if [ "$HAS_BACKEND" = true ]; then
    ...
    pm2 status | grep -E "multi-chart|name" || pm2 list
fi

echo ""
echo "=============================================="
echo -e "${GREEN}🎉 Multi Chart Deployment Complete!${NC}"
echo "=============================================="
echo ""
...
echo ""
echo "🌐 Access URLs:"
echo "  - Frontend: http://82.25.105.18/multi-chart/"
if [ "$HAS_BACKEND" = true ]; then
    echo "  - Backend API: http://82.25.105.18/api/multi-chart/"
fi
echo ""
if [ "$HAS_BACKEND" = true ]; then
