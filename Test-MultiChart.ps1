# Test Multi Chart Deployment
# Quick test script to verify Multi Chart is working

Write-Host "`n=== MULTI CHART DEPLOYMENT TEST ===`n" -ForegroundColor Cyan

$server = "82.25.105.18"

Write-Host "Testing Multi Chart deployment on $server...`n" -ForegroundColor Yellow

$tests = @(
    @{
        name = 'Frontend'
        url = "http://$server/multi-chart/"
        expectedCodes = @(200, 304)
    },
    @{
        name = 'Backend API'
        url = "http://$server/api/multi-chart/"
...
try {
    ssh root@$server "pm2 status | grep multi-chart || echo 'No PM2 process found'"
    Write-Host ""
} catch {
