...
function createMetricCell(value, type, label) {
    ...
    // Handle V1 display: convert decimals to whole numbers
    let displayValue = value;
    if (type === 'v1' && value !== null && value !== undefined) {
        if (value < 1 && value >= 0) {
    ...
    cell.setAttribute('data-tooltip', `${label}: ${displayValue}`);
    return cell;
}
...
function getColorClass(value, type) {
    ...
    if (type === 'ret') {
        ...
        // V1 (Percentile): Lower is BETTER (1 = best/lowest risk, 100 = worst/highest risk)
        // Reverse the colors: lower V1 = green (better), higher V1 = red (worse)
        if (value === null || value === undefined) return '';
        
        // Convert to 0-100 scale if value is between 0-1
        let scaledValue = value;
        if (value < 1 && value >= 0) {
            scaledValue = value * 100;
        }
        
        if (scaledValue >= 80) return 'red';        // 80-100: Red (worst)
        if (scaledValue >= 60) return 'orange';     // 60-79: Orange
        if (scaledValue >= 40) return 'yellow';     // 40-59: Yellow
        if (scaledValue >= 20) return 'light-green'; // 20-39: Light green
        return 'green-dark';                        // 0-19: Green (best)
    } else if (type === 'risk') {
