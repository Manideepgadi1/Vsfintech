...
# Riskometer SAFE Deployment Script
# This script will NOT disturb any existing applications
# Frontend: /riskometer/ | Backend Port: 5002 | API: /api/riskometer/
# Server: 82.25.105.18

set -e  # Exit on error

echo "🚀 Riskometer SAFE Deployment"
echo "======================================"
echo ""

# Configuration
APP_NAME="riskometer"
BACKEND_NAME="riskometer-backend"
APP_DIR="/var/www/vsfintech/Riskometer"
BACKEND_PORT=5002
...
if [ ! -d "$APP_DIR" ]; then
    ...
    echo "Please upload your Riskometer code to $APP_DIR first"
    exit 1
fi
...
if [ "$IS_REACT" = true ]; then
    ...
    echo "Configuring Vite for /riskometer subpath..."
    VITE_CONFIG=""
    ...
    if [ -n "$VITE_CONFIG" ]; then
        ...
        if grep -q "base:" "$VITE_CONFIG"; then
            sed -i "s|base:.*|base: '/riskometer',|" "$VITE_CONFIG"
        else
            sed -i "/export default defineConfig({/a\\  base: '/riskometer'," "$VITE_CONFIG"
        fi
    ...
    if [ -f "src/main.tsx" ]; then
        ...
        if grep -q "<BrowserRouter>" src/main.tsx && ! grep -q 'basename=' src/main.tsx; then
            sed -i 's|<BrowserRouter>|<BrowserRouter basename="/riskometer">|' src/main.tsx
    ...
    # Create .env file
    if [ "$HAS_BACKEND" = true ]; then
        echo "VITE_API_URL=/api/riskometer" > .env
...
if [ "$HAS_BACKEND" = true ]; then
    cat > /tmp/riskometer-nginx.conf << 'EOF'
...
EOF
    ...
    # =====================================================
    # RISKOMETER - Risk Assessment Tool (Static)
    # =====================================================
    location /riskometer/ {
        alias FRONTEND_PATH_PLACEHOLDER/;
        try_files $uri $uri/ /riskometer/index.html;
...
sed -i "s|FRONTEND_PATH_PLACEHOLDER|$FRONTEND_PATH|g" /tmp/riskometer-nginx.conf

# Check if Riskometer config already exists
if grep -q "RISKOMETER" "$NGINX_CONFIG"; then
    echo -e "${YELLOW}⚠️  Riskometer config already exists in nginx${NC}"
    ...
    if grep -q "location / {" "$NGINX_CONFIG"; then
        sed -i '/location \/ {/i\    # RISKOMETER CONFIGURATION' "$NGINX_CONFIG"
        sed -i '/# RISKOMETER CONFIGURATION/r /tmp/riskometer-nginx.conf' "$NGINX_CONFIG"
        sed -i '/# RISKOMETER CONFIGURATION/d' "$NGINX_CONFIG"
        echo -e "${GREEN}✓ Nginx configuration added${NC}"
    else
        echo -e "${YELLOW}⚠️  Could not automatically add nginx config${NC}"
        echo "Please manually add the config from: /tmp/riskometer-nginx.conf"
    fi
fi
...
# Test frontend
FRONTEND_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost/riskometer/ 2>/dev/null || echo "000")
...
if [ "$HAS_BACKEND" = true ]; then
    ...
    pm2 status | grep -E "riskometer|name" || pm2 list
fi

echo ""
echo "=============================================="
echo -e "${GREEN}🎉 Riskometer Deployment Complete!${NC}"
echo "=============================================="
echo ""
...
echo ""
echo "🌐 Access URLs:"
echo "  - Frontend: http://82.25.105.18/riskometer/"
if [ "$HAS_BACKEND" = true ]; then
    echo "  - Backend API: http://82.25.105.18/api/riskometer/"
fi
echo ""
if [ "$HAS_BACKEND" = true ]; then
