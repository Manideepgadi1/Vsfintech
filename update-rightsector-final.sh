...
# Complete Right Sector Update - Short Names with Hover + Correct Categories

set -e

echo "================================================"
echo " Right Sector Update - Categories & Short Names"
echo "================================================"
echo ""
...
# Create mappings
echo "[2/4] Creating mappings..."
cat > "${DATA_PATH}/index_mappings.json" << 'ENDJSON'
{
  "categories": {
    "BROAD": {
...
chown www-data:www-data "${DATA_PATH}/index_mappings.json" 2>/dev/null || true
chmod 644 "${DATA_PATH}/index_mappings.json"
echo "✓ Mappings created"

# List files
echo "[3/4] Right Sector files:"
find "$APP_PATH" -type f \( -name "*.html" -o -name "*.js" \) 2>/dev/null | head -10

echo ""
echo "[4/4] Complete!"
echo "✓ Categories: BROAD(26) SECTOR(20) STRATEGY(36) THEMATIC(44)"
echo "✓ Ready for HTML/JS update with short names + hover tooltips"
echo ""
